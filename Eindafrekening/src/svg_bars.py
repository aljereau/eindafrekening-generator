...
def generate_bar_svg(
    ...
    if is_overfilled:
        # OVERFLOW: Main bar fills pot_width only (overflow shown separately)
        total_width = pot_width  # Changed from pot_width + extension_width
    ...
    if is_overfilled:
        # When overfilled, the main bar should just fill the pot width (no extension)
        # The overflow is shown in a separate pill in the "Extra" column
